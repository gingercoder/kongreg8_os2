...
if($_POST['action'] == "add"){
    ...
    if($_POST['membername'] !=""){
        $memberlist = $objKidschurch->findMemberToAdd(db::escapechars($_POST['membername']), $groupid);
...
if($_GET['action'] == "add"){
    ...
    if(($_GET['m'] !="")&&($groupid !="")){
        ...
        $popuser = $objKidschurch->addMemberToGroup($groupid, db::escapechars($_GET['m']));
...
if($_GET['action'] == "remove"){
    if(($_GET['m'] !="")&&($groupid !="")){
        ...
        if($_GET['confirm'] == 'true'){
            $removemember = $objKidschurch->removeMemberFromGroup($groupid, db::escapechars($_GET['m']));
            ...
            $toggleMessage = "<p class=\"confirm\">Are you SURE you want to remove this member from the group?
                                <a href=\"index.php?mid=421&g=$groupid&action=remove&confirm=true&m=".db::escapechars($_GET['m'])."\">Yes, I agree</a>.
                                ";
        }
    }
    else{
...
if($_GET['action'] == "setleader"){
    ...
    $setleader = $objKidschurch->changeGroupLeader($groupid, db::escapechars($_GET['m']));
    if($setleader == 'true'){
...
require_once('modules/kidschurch/viewMembers.php');
