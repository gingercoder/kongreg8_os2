...
class groups extends kongreg8app{
    ...
    public function sendEmailCampaign($templateID)
    {
        ...
        if($groupID == 0){
            ...
            $sql = "SELECT * FROM (groupmembers JOIN churchmembers ON groupmembers.memberID = churchmembers.memberID) GROUP BY groupmembers.memberID";
        }
        else{
        ...
        foreach($result as $member){
            ...
            elseif($member['workemail'] !=''){
                $to = $member['workemail'];
                $sendme = 1;
            }
            if($sendme == 1){
                // Store the email in the email Temp system
                
                $sql2 = "INSERT INTO emailtemp SET
                        thesubject = '".$subject."',
                        thebody = '".$mainmessage."',
                        sendto = '".$to."',
                        mailstatus = 'pending',
        ...
        // run the send routine for the user
        $runsend = $this->sendEmail($this->usernametoid($_SESSION['Kusername']));
        if($runsend == FALSE){
            $errortoggle = 1;
        }
